...
correlation_matrix_diabetes = data[['diabetes'] + list(correlation_with_diabetes.index)].corr()
...
# Create age bins
age_bins = data['age'].value_counts(bins=5)
...
bubble_chart = px.scatter(diabetes_age_counts, x='age', y='diabetes_count', size='diabetes_count',
                          ...
                          title='Diabetes Count by Age', size_max=30)
...
correlation_chart_heatmap = px.imshow(correlation_matrix_diabetes, labels=dict(color="Correlation"),
...
scatter_plot = px.scatter(data_frame=data, x='age', y='bmi', color='diabetes', title='Age vs. BMI')

# Separate box plots for BMI, HbA1c level, and blood glucose level.
box_plot_bmi = px.box(data_frame=data, x='diabetes', y='bmi', color='diabetes', title='BMI')
box_plot_hba1c = px.box(data_frame=data, x='diabetes', y='HbA1c_level', color='diabetes', title='HbA1c level')
...
# Initial Histogram
initial_histogram = px.histogram(data_frame=data, x='age', title='Histogram')
# Initial dropdown for boxes
initial_box = px.box(data_frame=data, x='diabetes', y='bmi', color='diabetes', title='BMI')
...
def update_box_plot(selected_variable):
    fig = px.box(data, x='diabetes', y=selected_variable, color='diabetes',
                 title=f'{selected_variable.capitalize()}')
    return fig
...
def update_histogram(selected_variable):
    fig = px.histogram(data, x=selected_variable, title=f'Distribution of {selected_variable.capitalize()}')
    return fig
